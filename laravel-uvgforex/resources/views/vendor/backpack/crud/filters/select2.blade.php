...
<li filter-name="{{ $filter->name }}"
    ...
    <a href="#" class="nav-link dropdown-toggle" data-toggle="dropdown" role="button" aria-haspopup="true" aria-expanded="false"><span id="filter_name_{{ $filter->name }}" class="{{ $filter->label }}">{{ $filter->label }} </span><span class="caret"></span></a>
        ...
        jQuery(document).ready(function($) {
            ...
            $('select[data-filter-type=select2]').not('[data-filter-enabled]').each(function () {
            	...
            	var obj = $(this).select2({
					...
					var val = $.trim($("#filter_"+filterName+" :selected").text());
					var className = $("#filter_name_"+filterName).attr('class') 
                    if(val != '-'){
						$("#filter_name_"+filterName).text(val);
					} else {
						$("#filter_name_"+filterName).text(className);
					}
					if(!value) {
                       return;
                    }
