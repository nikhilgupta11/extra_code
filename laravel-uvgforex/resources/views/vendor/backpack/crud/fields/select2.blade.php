    ...
    <select
        ...
        @if (count($options))
            @foreach ($options as $option)
                @if(Request::route()->getName() == 'currency.edit')
                    @if($current_value == $option->getKey())
                        <option value="{{ $option->getKey() }}" selected {{$option->status == 0 ? 'disabled' : ''}}>{{ $option->{$field['attribute']} }}</option>
                    @else
                        <option value="{{ $option->getKey() }}" {{$option->status == 0 ? 'disabled' : 'false'}}>{{ $option->{$field['attribute']} }}</option>
                    @endif
                @else
                    @if($current_value == $option->getKey())
