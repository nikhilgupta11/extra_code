...
if(! function_exists('generateCertificateNumber')){
    function generateCertificateNumber($project) {
        $number = $project->certification_start_txt.mt_rand($project->certification_start_number, $project->certification_end_number).$project->certification_end_txt;
    
        if (certificateNumberExists($number)) {
            return generateCertificateNumber($project);
        }
    ...
    function certificateNumberExists($number) {
        return Payment::where('certificate',$number)->exists();
    }
}
/*
 * Global helpers file with misc functions.
...
if (! function_exists('show_column_value')) {
    ...
    function show_column_value($valueObject, $column, $return_format = '')
    {
        ...
        } else if($column_type != 'json' && filter_var($value, FILTER_VALIDATE_URL) && strpos($value,pathinfo($value, PATHINFO_EXTENSION))) {
            $return_text = '<figure class="figure">
            <a href="'.$value.'" data-lightbox="image-set" data-title="Path: '.$value.'">
                <img src="'.$value.'" style="max-width:200px;" class="figure-img img-fluid rounded img-thumbnail" alt="">
            </a>
            </figure>';
        } else {
            $return_text = $value;
...
if (! function_exists('encode_id')) {
    ...
    function encode_id($id)
    {
        ...
        $hashid = 'zXfMPtcERG7XVFF4yUsSaRgOg';
        return $hashid;
    }
}
